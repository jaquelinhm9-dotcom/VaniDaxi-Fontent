...
export default function AdminPortal({children}){
 ...
 useEffect(()=>{let alive=true;const check=async()=>{const s=session(),id=s?.user?.id;if(!id){if(alive){setRole('');setOpen(false)}return}try{const r=await fetch(`${URL}/rest/v1/profiles?select=role,is_active&id=eq.${encodeURIComponent(id)}&limit=1`,{headers:{apikey:KEY,Authorization:`Bearer ${s.access_token||KEY}`}});const rows=await r.json();if(alive)setRole(rows?.[0]?.is_active?rows?.[0]?.role||'':'')}catch{if(alive)setRole('')}};check();const t=setInterval(check,2500);return()=>{alive=false;clearInterval(t)}},[]);
