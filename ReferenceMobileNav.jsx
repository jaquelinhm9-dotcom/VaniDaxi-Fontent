...
import ReferenceMobileHome from "./ReferenceMobileHome.jsx";
...
export default function ReferenceMobileNav() {
  ...
  return (
    <>
      {home && <ReferenceMobileHome />}
      <nav className="reference-mobile-nav" aria-label="Navegación principal">
        {items.map((item) => (
          <NavLink key={item.to} to={item.to} end={item.end} className={({ isActive }) => `reference-mobile-nav-item ${isActive ? "active" : ""}`}>
            <span className="reference-mobile-nav-icon"><Icon name={item.icon} /></span>
            <span>{item.label}</span>
          </NavLink>
        ))}
      </nav>
      <span className="reference-cart-state" aria-hidden="true">{cartCount}</span>
    </>
  );
}
