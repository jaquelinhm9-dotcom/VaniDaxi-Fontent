...
export default function VaniDaxiShell(){
 const[route,setRoute]=useState(()=>location.hash||'#/home'),[role,setRole]=useState(''),[categories,setCategories]=useState([]),[authenticated,setAuthenticated]=useState(hasSession)
 useEffect(()=>{const onHash=()=>{setRoute(location.hash||'#/home');setAuthenticated(hasSession())};addEventListener('hashchange',onHash);addEventListener('popstate',onHash);return()=>{removeEventListener('hashchange',onHash);removeEventListener('popstate',onHash)}},[])
 useEffect(()=>{let alive=true;const check=async()=>{const s=session(),id=s?.user?.id,ok=hasSession();if(alive)setAuthenticated(ok);if(!id){if(alive){setRole('');setCategories([])}return}try{const[r,c]=await Promise.all([fetch(`${URL}/rest/v1/profiles?select=role,is_active&id=eq.${encodeURIComponent(id)}&limit=1`,{headers:{apikey:KEY,Authorization:`Bearer ${s.access_token||KEY}`}}),fetchCatalog()]);const rows=await r.json();if(alive){setRole(rows?.[0]?.is_active?rows?.[0]?.role||'':'');setCategories(c?.categories||[])}}catch{if(alive)setRole('')}};check();const onStorage=e=>{if(e.key==='vanidaxi-auth-session')setAuthenticated(hasSession())};addEventListener('storage',onStorage);return()=>{alive=false;removeEventListener('storage',onStorage)}},[])
