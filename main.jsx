...
function showLoading() {
  ...
  root.innerHTML = '<main style="min-height:100vh;display:grid;place-items:center;padding:24px;background:#faf7fc;color:#2a2030;font-family:system-ui,sans-serif"><section style="text-align:center"><div style="font-size:32px;font-weight:800;letter-spacing:-.04em">VaniDaxi</div><p style="color:#756b77">Cargando tu tienda…</p></section></main>'
}
...
async function bootstrap() {
  try {
    showLoading()
    startVaniDaxiSync()
    const hydration = hydrateVaniDaxi().catch(() => null)
    const catalog = await Promise.race([fetchCatalog().catch(() => ({ categories: [], products: [] })), timeout(1200)])
    window.__VANI_COMMERCIAL_CATALOG__ = catalog?.products || []
    window.__VANI_COMMERCIAL_CATEGORIES__ = catalog?.categories || []

    const [{ default: App }] = await Promise.all([import('./App.jsx'), hydration])
